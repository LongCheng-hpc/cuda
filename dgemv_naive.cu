...
__global__ void kernel(float * A, float * x, float * b, int M, int N) {
...
// void cudaDGEMV(float * A, float * x, float * b, int M, int N) {
// // void cudaDGEMV(float * A, float * x, float * b, int M, int N, dim3 numBlock, dim3 ThreadsPerBlock) {
// 	int tid;
// 	// Init three vectors on the gpu to store the matrix
// 	float *d_A, *d_x, *d_b;
...
int main(int argc, char ** argv) {
	...
	//Assume a 1D grid
	gridX = 128; 	//TODO: Select the number of blocks
	gridY = 1; 
	gridZ = 1;
	...
	//TODO - Question 3: Call the "cudaDGEMV" function
		//TODO: Call cudaDGEMV
	// cudaDGEMV(A, x, sol, M, N, numBlock, ThreadsPerBlock);
	// cudaDGEMV(A, x, sol, M, N);
	kernel<<<numBlock, ThreadsPerBlock>>>(dev_A, dev_x, dev_b, M, N);
	//----
	...
	cudaMemcpy(b, dev_b, sizeof(float) * M, cudaMemcpyDeviceToHost);
	//----
